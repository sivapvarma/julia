...
# type predicates
datatype_alignment(dt::DataType) = Int(unsafe_load(convert(Ptr{DataTypeLayout}, dt.layout)).alignment & 0x0FFFFFFF)
datatype_haspadding(dt::DataType) = (unsafe_load(convert(Ptr{DataTypeLayout}, dt.layout)).alignment >> 28) & 1 == 1
datatype_pointerfree(dt::DataType) = (unsafe_load(convert(Ptr{DataTypeLayout}, dt.layout)).alignment >> 29) & 1 == 1
datatype_fielddesc_type(dt::DataType) = (unsafe_load(convert(Ptr{DataTypeLayout}, dt.layout)).alignment >> 30) & 3
...
isbits(t::DataType) = (@_pure_meta; !t.mutable & t.isleaftype && datatype_pointerfree(t))
