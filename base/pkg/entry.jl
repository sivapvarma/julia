...
function update(branch::AbstractString, upkgs::Set{String})
    ...
    if !isempty(upkgs)
        for (p, (v,f)) in instd
            satisfies(p, v, reqs) || throw(PkgError("Package $pkg: current package status does not satisfy the requirements, cannot do a partial update; use Pkg.update()"))
        end
    end
...
function resolve(
    ...
    Query.check_partial_updates(orig_reqs,orig_deps,want,upkgs)
